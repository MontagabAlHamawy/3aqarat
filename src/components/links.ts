
export const Imag = [
  { name: "/home/building.png" },
  { name: "/home/psd.jpg" },
  { name: "/home/land.jpg" },
  { name: "/home/rr.png" },
  { name: "/home/we.png" },
];

export const ImagBuilding = [
  { photo: "/home/building.png" },
];
export const ImagApartment = [
  { photo: "/home/apartment.png" },
];
export const ImagHouse = [
  { photo: "/home/house.png" },
];
export const ImagLand = [
  { photo: "/home/land.png" },
];
export const ImagCommercials = [
  { photo: "/home/commercial.png" },
];

export const hom = [
  { name: "الكل", link: "/propertys/", type: "all" },
  { name: "المنازل", link: "/propertys/houses", type: "house" },
  { name: "الشقق", link: "/propertys/apartments", type: "apartment" },
  { name: "المحال التجارية", link: "/propertys/commercials", type: "commercial" },
  { name: "الأراضي", link: "/propertys/lands", type: "land" },
  { name: "المحاضر", link: "/propertys/buildings", type: "building" },
];
export const Blogs = [
  { id: "types-of-property-ownership", title: "أنواع ملكية العقار في سوريا", description: "هناك العديد من أنواع الملكيات العقارية في سورية وقسم كبير من الناس ربما لا تعي أو تعلم الفرق بين هذه التسميات في الملكية, لذلك سنقوم بشرحها بطريقة سهلة وبسيطة ليفهمها عامة الناس وخاصة من ناحية المخاطر والحجية القانونية", image: "/types-of-property-ownership.png" },
];
export const whyus = [
  {
    title: "تفاصيل شاملة",
    discrep:
      "يمكنك ان تتعرف على كافة التفاصيل الخاصة بالعقار الذي تبحث عنه بداية من المساحة وعدد الغرف والسعر وقيمة القسط الشهري  وموعد الاستلام",
  },
  {
    title: "سهولة الاستخدام",
    discrep:
      "يمكنك ان  تختار ترتيب العقارات وفقا للاقل سعرا او لاطول فترة سداد ممكنةاو الحاهزة للسكن وغيرها  من التفاصيل التي تحتاج اليها ",
  },
  {
    title: "حجز مباشر",
    discrep: "يمكنك ان تقوم بحجز وحدتك عبر الموقع الالكتروني بضغطة واحدة",
  },
  {
    title: "أماكن مميزة",
    discrep: "يمكنك البحث عن العقار في المناطق التي تفضل السكن فيها",
  },
];


export function formatNumber(num: number): string {
  const numStr = num.toString();
  const formattedNumStr = numStr.replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  return formattedNumStr;
}

export function truncateText(text: string, maxLength: number): string {
  if (text.length <= maxLength) {
      return text;
  }
  return text.substring(0, maxLength) + '...';
}

