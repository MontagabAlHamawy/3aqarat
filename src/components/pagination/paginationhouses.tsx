...
export default function PaginationHouses({ page }: PaginationProps) {
  ...
  return (
    <div className="flex flex-row justify-center items-center w-max px-8 py-2 rounded-lg gap-10 bg-sidpar">
      <Link href={`/propertys/houses/${path1}`} className="p-2 rounded-lg text-xl xl:text-2xl bg-accent cursor-pointer">
      <PiCaretRightDuotone />
      <p className="text-base">التالي</p>
      </Link >
      <Link href={`/propertys/houses/${path2}`} className="p-2 text-xl xl:text-2xl rounded-lg bg-accent cursor-pointer">
