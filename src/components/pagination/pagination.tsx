...
export default function Pagination({ page }: PaginationProps) {
  ...
  return (
    <div className="flex flex-row justify-center items-center w-max px-8 py-2 rounded-lg gap-10 bg-sidpar">
      <Link href={`/buildings${path1}`} className="p-2 flex justify-between items-center rounded-lg text-xl xl:text-2xl bg-accent cursor-pointer">
        <PiCaretRightDuotone />
        <p className="text-base">التالي</p>
      </Link>
      <Link href={`/buildings${path2}`} className="p-2 flex justify-between items-center text-xl xl:text-2xl rounded-lg bg-accent cursor-pointer">
