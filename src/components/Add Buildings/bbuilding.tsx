  ...
  const onSubmit = async (data: any) => {
    await showConfirmation(async () => {
      ...
      let bodyContent = {
        ...
        num_of_apartments: Number(data.rooms),
        direction: data.direction,
        num_of_floors: Number(data.floorNumber),
      };

      try {
        const response = await axios.post(`${apiUrl}/buildings/`, bodyContent, {
          headers: headersList,
        });
      ...
      <form className="flex flex-col justify-center items-center" onSubmit={handleSubmit(onSubmit)}>
        <div className="">
          ...
          <div className="flex flex-row justify-center items-center gap-1 xl:gap-14">
            ...
            <div className="mb-4">
              <label className="block text-white font-semibold text-sm mb-2">عدد الشقق :</label>
              <input
                type="text"
                {...register("rooms", { required: "هذا الحقل مطلوب" })}
          ...
          <div className="flex flex-row justify-center items-center gap-1 xl:gap-14">
            <div className="mb-4">
              ...
              <input
                type="text"
                {...register("floorNumber", { required: "هذا الحقل مطلوب" })}
                placeholder="عدد الطوابق"
                className="w-40 xl:w-[397px] border p-2 rounded-lg bg-section border-section text-white"
              />
              {errors.floorNumber && <p className="text-red-500">{errors.floorNumber.message}</p>}
            </div>
            <div className="mb-4">
